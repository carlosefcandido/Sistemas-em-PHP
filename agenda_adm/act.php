<?php
	...
	switch($acao)
	{
			...
			case "abrirAgendaDia";
					...
					$dadosImpresso = $calendario->criaDia($_GET['data'],$_GET['medid']);
					echo $dadosImpresso;					
					break;
					
			case "confirmaratendimento":
					include "include/atendimento/confirmaratendimento.php";		
					break;
		...
		case "edittableprocedimentoview":
			...
			case "visualizarlaudo":
					...
					$local = $laudo->getCampo("local");
					//print"<script>alert('Meu nome é $local')</script>";
					...
					if(!$erro){
						$ftp = new ftp($arquivo,$laudo->getCampo("local"));
						if($laudo->getCampo("local") == 'Hospital de Clinicas Dr. Aloa'){
							$dirLocal = "./laudo/arquivos/".$arquivo;					
						}else{
							$dirLocal = "prontolab/arquivos/".$arquivo;					
						}

						if (file_exists($dirLocal)) {
							$laudodao = new classLaudoDAO();
							$laudodao->gravar($laudo->getCampo("local"));
							$xml = simplexml_load_file($dirLocal);
						} else {
							$erro = "Erro | Exame em análise.";
						}
						if(!$erro){
							if($xml){
								if($xml[0]->Atendimento->Cliente->Senha == $_GET['senha']){
									...
									if($laudo->getCampo("local") == 'Hospital de Clinicas Dr. Aloa'){
										$erro = "Ok | http://mprado.info/agenda/laudo/abrirlaudo.php";
									}else{
										$erro = "Ok | http://mprado.info/agenda/prontolab/abrirlaudo.php";
									}									
	...
	case "Acessar":
			...
			if (!$usuario->getCampo("login")){
				...
				$usuarioDAO = new classUsuarioDAO();
				$msg = $usuarioDAO->logarParaLaudar($usuario);
			}
	...
	case "adicionarlaudo":
			include "gerarlaudo/include/laudo/adicionarlaudo.php";		
			break;
